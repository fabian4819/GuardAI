...
var configSchema = exports_external.object({
  ...
  vaultSentinelAddress: exports_external.string(),
  chainName: exports_external.string(),
  gasLimit: exports_external.string(),
  riskThreshold: exports_external.number(),
  ethPriceFeedAddress: exports_external.string(),
  priceBaseline: exports_external.number(),
  lidoTvlBaseline: exports_external.number(),
  aaveTvlBaseline: exports_external.number(),
  makerTvlBaseline: exports_external.number()
});
var fetchEthPrice = (sendRequester, _config) => {
...
var fetchLidoTvl = (sendRequester, _config) => {
  const resp = sendRequester.sendRequest({ method: "GET", url: "https://api.llama.fi/protocol/lido" }).result();
  if (resp.statusCode !== 200) {
...
var fetchAaveTvl = (sendRequester, _config) => {
  const resp = sendRequester.sendRequest({ method: "GET", url: "https://api.llama.fi/protocol/aave" }).result();
  if (resp.statusCode !== 200) {
...
var fetchMakerTvl = (sendRequester, _config) => {
  const resp = sendRequester.sendRequest({ method: "GET", url: "https://api.llama.fi/protocol/makerdao" }).result();
  if (resp.statusCode !== 200) {
...
var computeRiskScore = (ethPrice, lidoTvl, aaveTvl, makerTvl, config) => {
  ...
  const riskScore = Math.round(priceScore * 0.3 + lidoScore * 0.25 + aaveScore * 0.25 + makerScore * 0.2);
  return {
    riskScore,
    ethPrice,
...
var encodeSetRiskScore = (score) => encodeFunctionData({
  abi: VAULT_SENTINEL_ABI,
  functionName: "setRiskScore",
  args: [BigInt(score)]
});
var writeRiskScoreViaRpc = (sendRequester, config, score) => {
...
var doRiskMonitor = (runtime2) => {
  const http2 = new cre.capabilities.HTTPClient;
  const { ethPrice } = http2.sendRequest(runtime2, fetchEthPrice, ConsensusAggregationByFields({ ethPrice: median }))(runtime2.config).result();
  const { lidoTvl } = http2.sendRequest(runtime2, fetchLidoTvl, ConsensusAggregationByFields({ lidoTvl: median }))(runtime2.config).result();
  const { aaveTvl } = http2.sendRequest(runtime2, fetchAaveTvl, ConsensusAggregationByFields({ aaveTvl: median }))(runtime2.config).result();
  const { makerTvl } = http2.sendRequest(runtime2, fetchMakerTvl, ConsensusAggregationByFields({ makerTvl: median }))(runtime2.config).result();
  const breakdown = computeRiskScore(ethPrice, lidoTvl, aaveTvl, makerTvl, runtime2.config);
  const { riskScore } = breakdown;
  runtime2.log(`[VaultSentinel] ═══ SYSTEMIC RISK INDEX ═══`);
  runtime2.log(`  SCORE       : ${riskScore}/100 ${riskScore >= runtime2.config.riskThreshold ? "⚠️  EMERGENCY!" : "✅ SAFE"}`);
  runtime2.log(`  ETH Price   : $${breakdown.ethPrice.toFixed(2)} → priceScore: ${breakdown.priceScore}`);
  runtime2.log(`  Lido TVL    : $${breakdown.lidoTvlB}B → lidoScore:  ${breakdown.lidoScore}`);
  runtime2.log(`  Aave TVL    : $${breakdown.aaveTvlB}B → aaveScore:  ${breakdown.aaveScore}`);
  runtime2.log(`  MakerDAO    : $${breakdown.makerTvlB}B → makerScore: ${breakdown.makerScore}`);
  if (riskScore >= runtime2.config.riskThreshold) {
    runtime2.log(`  \uD83D\uDEA8 THRESHOLD BREACHED — setRiskScore(${riskScore}) will trigger emergency!`);
  }
  const { txHash } = http2.sendRequest(runtime2, writeRiskScoreViaRpc, ConsensusAggregationByFields({ txHash: identical }))(runtime2.config, riskScore).result();
  runtime2.log(`✅ setRiskScore(${riskScore}) → txHash: ${txHash}`);
...
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`[VaultSentinel] CronTrigger fired`);
  return doRiskMonitor(runtime2);
};
